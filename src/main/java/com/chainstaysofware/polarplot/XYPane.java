...
import com.chainstaysofware.polarplot.data.XYItem;
import com.chainstaysofware.polarplot.font.Fonts;
import com.chainstaysofware.polarplot.series.XYSeries;
import com.chainstaysofware.polarplot.tools.Helper;
import com.chainstaysofware.polarplot.tools.Point;
...
import static com.chainstaysofware.polarplot.ChartType.INTERPOLATE_POLAR;
import static com.chainstaysofware.polarplot.tools.Helper.clamp;
...
public class XYPane<T extends XYItem> extends Region implements ChartArea {
   ...
   private void drawPolar(final XYSeries<T> series,
      ...
      if (INTERPOLATE_POLAR == series.getChartType()) {
         //noinspection unchecked
         points = buildPoints(series, xCenter, yCenter, yLowerBound,
            yRange, range, offset);
         @SuppressWarnings("unchecked") final var interpolatedItems = interpolate((List<XYChartItem>) series.getItems());
         //noinspection unchecked
   ...
   private Point[] buildPoints(XYSeries<T> series,
      ...
      return buildPoints(items, xCenter, yCenter, yLowerBound, yRange, range, offset);
   }
...
//      points[points.length - 1] = points[0]; // last point == first point
    ...
    * TODO: allow configuration on how many points to generate.
    */
   private List<XYChartItem> interpolate(final List<XYChartItem> points) {
      if (points.size() < 2) {
         return points;
      }
   ...
   private List<XYChartItem> interpolate(final XYChartItem p1,
   ...
   private XYChartItem interpolate(final double r1,
                                   final double theta1,
                                   final double r2,
                                   final double theta2,
                                   final double theta) {
   ...
   private void drawPolarGrid(final double angleStep,
      ...
      for (int i = 0; i < numSectors; i++) {
         ctx.fillText(String.format(Locale.US, "%.0f", i * angleStep), xCenter, size * 0.02);
         Helper.rotateCtx(ctx, xCenter, yCenter, angleStep);
      }
      ctx.restore();
   }
